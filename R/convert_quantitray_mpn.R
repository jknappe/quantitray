...
#' @export
#' @examples
#' convert_quantitray_mpn(large = 42, method = "qt")
#' convert_quantitray_mpn(large = 42, small = 42, method = "qt-2000")
#' convert_quantitray_mpn(large = 5, small = 42, method = "qt-legio")
#'

convert_quantitray_mpn <- function(large, small, method) {
  #
  # Error handling
  # ~~~~~~~~~~~~~~~~
  #
  ...
  if (!missing(method) & !method %in% c("qt", "qt-2000", "qt-legio")) {
    stop("'method' must have value 'qt', 'qt-2000', or 'qt-legio'.")
  }
  #
